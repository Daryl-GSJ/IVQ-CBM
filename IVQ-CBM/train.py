...
def train_net(model, config):
    ...
    for epoch in range(config.epochs):
        ...
        if val_BMAC >= best_bmac_val:
            ...
            torch.save(model.state_dict(), f'%s%s/best_ckpt.pth'%(config.cp_path, config.unique_name))
        ...
        if epoch % 10 == 0:
            torch.save(model.state_dict(), f'%s%s/{epoch}_ckpt.pth'%(config.cp_path, config.unique_name))
        ...
        print(output)
    # Save rank_epochs data to current directory
    rank_epochs_data = {
        'train_ranks': rank_epochs[0],
        'val_ranks': rank_epochs[1]
    }
    np.save('rank_epochs_ivqcbm.npy', rank_epochs_data)
...
if __name__ == '__main__':
    ...
    parser.add_option('-b', '--batch_size', dest='batch_size', default=32, # 128
    ...
    parser.add_option('-m', '--model', type='str', dest='model',
            default='ivq_cbm', help='use which model')
    ...
    parser.add_option('--data-path', type='str', dest='data_path', 
            default='IVQ-CBM/dataset/isic2018/', help='the path of the dataset')
    parser.add_option('-u', '--unique_name', type='str', dest='unique_name',
            default='test', help='name prefix')
    parser.add_option('--flag', type='int', dest='flag', default=2) # 2默认值
    ...
    from concepts import isic_concept_dict
    concept_list = isic_concept_dict
    net = IVQ_CBM(concept_list=concept_list, model_name='biomedclip', config=config)
